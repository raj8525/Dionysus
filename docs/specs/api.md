...
每条 E2E case 必须可记录执行结果：`passed`、`failed`、`blocked`、`skipped`，同时保存失败原因与证据 JSON。Campaign 状态必须由 case 状态自动汇总，不能靠人工口头判断。`passed` 结果必须包含 `mode=strict` 或 `mode=render-only`、`targetUrl`、`screenshotPath` 和 `consoleErrors[]`，否则 API 必须拒绝。`render-only` 只用于工程诊断；`milestone verdict passed` 必须进一步要求所有已通过 case-result 都是 `mode=strict`。
...
Agent Runtime 必须在隔离工作区运行 Worker / RuleWriter / TestWriter。目标项目 worktree 在 Agent 执行期间发生变化时，必须区分“其他 integration 正常落地”和“当前 Agent 绕过 workspace 直接写目标项目”。
...
`modelCalls` 的口径必须优先读取 PostgreSQL `task_runs.model_call_count`。如果 CLI 输出包含形如 `DIONYSUS_USAGE_JSON={"modelCalls":3}` 或 `DIONYSUS_DONE_JSON={"status":"done","modelCalls":3}` 的回执，Agent Runtime 必须把它持久化到 `task_runs.model_call_count` 与 `task_runs.model_usage_json`，前端和 CLI 统计使用该真实值。真实 Agent 仍必须输出精确 marker 行；Runtime 额外容忍常见 Markdown 整行包裹，例如 `**DIONYSUS_DONE_JSON=...**`，防止 OpenCode/MiniMax 把最终标记加粗后导致模型调用统计缺失或 CLI 进程无法及时收口。缺少 usage 回执时，Dionysus 才回退为估算：非 `mock` 的 CLI run 按 1 次模型调用计，`mock` 按 0 次计。
...
`system audit` 是 Codex 的高层运行判断入口。它必须合并 `system readiness`、`agent usage`、pending `codex_outbox`、目标项目未关闭 goal 和可选 goal 聚合状态，返回 `ready` / `needs_attention` / `blocked`、`blockers`、`warnings`、`notes`、`nextAction`、`nextCommands` 和 evidence。`readiness.blocked` 必须直接阻断派工；pending outbox、真实模型调用证据缺失、运行中调用、当前高失败率角色、陈旧未关闭 goal、以及某角色在未关闭 goal 中最近一次 CLI 运行失败且尚无后续成功恢复证据时，必须进入 `needs_attention`，由 Codex 先处理风险再扩大并发。`agent usage` 必须暴露全历史最后成功 / 失败时间与未关闭 goal 的最新运行时间；如果某角色历史失败率偏高但最近一次运行已经成功，audit 必须将其降级为 `notes`；如果失败属于已 `done` / `failed` / `cancelled` 的历史 goal，则保留在总成本统计中，但不得作为当前派工风险永久阻断。陈旧 `created` / `fast_lane` / 其他未终态 goal 会污染 active-goal 统计和 Dashboard 判断，必须在 audit 中暴露为 `staleOpenGoals`，并给出 `goal cancel` 收口命令。
...
对于 `fast_lane` goal，`supervise` 每轮必须先读取 `GET /api/goals/:id/status` 并复用 `fastlane status` 的 phase 判断。如果 phase 为 `ready_for_data_followups` 或 `ready_for_reviewer`，且存在可入队任务，`supervise` 必须自动执行等价于 `fastlane advance` 的安全推进，并继续下一轮。其他 phase 不得自动推进：`reviewer_review` 必须等待 Codex 审查 Reviewer 分数，`codex_final` 必须等待 Codex 测试/E2E/发布，`e2e_required` 必须交给 Codex 浏览器验收。`reviewer_review` 和 `codex_final` 必须返回 `codex_required`，不能误报为 `blocked/no active Dionysus work`；这表示系统正常停在 Codex 裁决点，而不是 Agent 队列故障。
...
- Worker Runtime 必须从任务描述中的 `Allowed files:`、`Allowed paths:`、`允许修改路径:`、`允许修改文件:`、`文件范围:` 或 `只允许修改:` 提取范围，并随 patch 写入 `patches.allowed_files_json`。解析必须支持单层仓库目录范围，例如 `migrations/`、`features_test/`、`docs/`，不得只识别 `docs/specs/` 这类多层路径。
