...
对于 `fast_lane` goal，`supervise` 每轮必须先读取 `GET /api/goals/:id/status` 并复用 `fastlane status` 的 phase 判断。如果 phase 为 `ready_for_data_followups` 或 `ready_for_reviewer`，且存在可入队任务，`supervise` 必须自动执行等价于 `fastlane advance` 的安全推进，并继续下一轮。其他 phase 不得自动推进：`reviewer_review` 必须等待 Codex 审查 Reviewer 分数，`codex_final` 必须等待 Codex 测试/E2E/发布，`e2e_required` 必须交给 Codex 浏览器验收。
