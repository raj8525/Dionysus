...
## 2026-05-20 Dionysus Fast Lane 产品语义门禁修复记录

### 背景

- Coupon `/identity` 试点暴露出 Dionysus 对 WorkerCLI / ReviewerCLI 的 UI 迁移门禁仍不够具体：低成本 Worker 容易机械参考 HTML 模板，导致 Vue 页面外观接近但功能入口、链接、按钮和列表详情联动被破坏。
- 用户进一步明确：不能教条地 100% 复刻 HTML，也不能教条地把所有点击都改成不跳转；必须站在系统功能、系统设计和最终用户体验的产品经理视角设计 UI/UX。

### 完成内容

- `tools/dionysus-fastlane.ts`
  - `coupon-module-plan` / `coupon-module-start` 对非成熟一级 Vue 页面自动推断 HTML 模板，例如 `apps/admin-web/src/pages/identity.vue` 自动对照 `apps/admin-web/html/identity.html`。
  - `tenants.vue` / `hotels.vue` 保持成熟页面保护，不注入模板重写门禁。
  - 新增 HTML 模板结构一致性、视觉风格、产品语义、功能入口保真和浏览器证据门禁。
  - 产品语义门禁明确要求区分两类点击：
    - 对象行、Tab、筛选 chip、详情卡片等上下文选择入口，优先页内更新右侧/下方详情。
    - 进入完整管理页、新增、编辑、审批、审计详情、导出等明确 CTA，才跳转子页面或打开真实弹窗。
- `tools/dionysus-fastlane.test.ts`
  - 新增/扩展单测，锁定非成熟页面注入模板门禁、成熟 tenants/hotels 页面不注入门禁、自动推断模板路径、Reviewer 90 分门禁必须检查产品语义。
- `skills/dionysus/SKILL.md`
  - 更新 Codex 使用 Dionysus 的 Skill，明确 HTML 模板只是视觉和信息架构参考，不是不可变交互脚本。
  - 要求 WorkerCLI / ReviewerCLI 同时验证页内上下文切换和明确 CTA 路由/弹窗。
- `AGENTS.md`
  - 补充 fast lane 页面任务的产品语义规则。
- `docs/specs/api.md`
  - 增补 `coupon-module-plan` 的页面迁移契约：模板一致性、产品语义、功能入口保真、E2E/截图证据是 Reviewer 90 分门禁的一部分。
- `features_test/dionysus-mvp.feature.md`
  - 补充 BDD 场景：Coupon 模块计划必须要求 Worker 区分页内上下文切换和明确 CTA 跳转。
- `packages/cli-adapters/src/probe.test.ts`
- `packages/cli-adapters/src/real-cli-adapters.test.ts`
  - 将真实子进程 probe / Git Guard 测试 timeout 从默认 5s 放宽到 10s。
  - 根因：这些测试启动真实 Node 子进程与 detached process group，在本机全量并发测试时接近或超过 Vitest 默认 5s；手工 adapter 复现逻辑正确，超时属于测试环境上限过窄。

### 验证证据

- `pnpm exec vitest run tools/dionysus-fastlane.test.ts` 通过，20/20。
- `pnpm typecheck` 通过。
- `pnpm -s dionysus fastlane coupon-module-plan --module "身份权限" ... --page apps/admin-web/src/pages/identity.vue --api /api/admin/identity/overview` 输出已包含：
  - `产品语义门禁`
  - `不得机械 100% 复刻 HTML`
  - `页内上下文切换不突兀跳转`
  - `明确 CTA 能进入对应完整子页面/弹窗`
  - Reviewer 不能用“所有点击都不跳转”或“所有模板链接都照抄跳转”这种机械规则放行。
- `pnpm exec vitest run packages/cli-adapters/src/probe.test.ts packages/cli-adapters/src/real-cli-adapters.test.ts tools/dionysus-fastlane.test.ts` 通过，32/32。
- `pnpm test` 通过，50 个测试文件、246 个测试。

### 下一步

- 提交并推送本轮 Dionysus 改动。
- 若 Dionysus runtime 正在运行，提交后执行 `pnpm -s dionysus system runtime heal`，确保 API / Worker 使用新 commit。
- 后续继续推进 Dionysus 到真实 Coupon 模块开发：优先让 fast lane 可针对 D1 子模块创建可执行任务、记录 CLI usage、产出 ReviewerCLI 分数和 Codex release record。

## 2026-05-20 Codex Complete 运行状态修复记录

### 背景

- 在 Coupon 目标 `506b11c7-9309-4f38-af0c-2092bb6d575d` 收口时，Codex 尝试对 Master review 任务执行 `task codex-complete`。
- API 返回 500：`new row for relation "task_runs" violates check constraint "task_runs_status_check"`。
- 根因是 `completeTaskByCodex` 在关闭仍处于 `running` 的 `task_runs` 时写入了 `status = 'passed'`。
- `passed` 是 milestone / release 的状态词，不属于 `task_runs.status` 合法枚举；`task_runs` 的成功完成状态应为 `succeeded`。

### 修复

- `packages/core/src/state-machine.ts`
  - 新增 `taskRunStatusForCodexCompletion()`，统一返回 `succeeded`，避免把 milestone/release 状态词误用到 run 状态。
- `packages/db/src/repository.ts`
  - `completeTaskByCodex` 关闭 running run 时改用 `taskRunStatusForCodexCompletion()`。
- `packages/core/src/state-machine.test.ts`
  - 增加单测，锁定 Codex handoff 使用 `task_runs` 状态词 `succeeded`。
- `docs/specs/state-machine.md`
  - 补充契约：`task codex-complete` 必须将 task 标记为 `done`，并将仍处于 `running` 的 `task_runs` 收口为 `succeeded`，不得写入 `passed`。

### 验证

- `pnpm vitest run packages/core/src/state-machine.test.ts` 通过，7/7。
- `pnpm typecheck` 通过。

### 后续

- 仍需跑 `pnpm test` 全量验证。
- 提交后需要执行 `pnpm -s dionysus system runtime heal`，确保运行中的 API / Worker 加载修复后的 commit。

## 2026-05-20 Dionysus 系统审计入口记录

### 背景

- 用户要求后续开发必须站在系统功能、系统设计、用户体验和产品经理全局视角推进，不能教条执行单点命令。
- Dionysus 当时已经具备 `doctor`、`readiness`、`agent usage`、`codex heartbeat` 等碎片化命令，但 Codex 仍需要人工拼接判断“现在能不能派活、风险在哪、下一步做什么”。
- 这种碎片化判断会放大 Paperclip / Multica 式监管失效：系统看似在跑，但高失败率角色、pending Codex 介入事件、真实 CLI / 模型调用证据缺口可能没有被作为产品风险拦截。

### 完成内容

- 新增 `tools/dionysus-system-audit.ts`
  - 提供 `buildSystemAuditSummary()`，合并 readiness、Agent CLI usage、pending Codex outbox 和可选 goal status。
  - 输出固定为 `ready` / `needs_attention` / `blocked`。
  - readiness blocker 直接阻断派工。
  - pending Codex outbox、真实 CLI / 模型调用证据缺失、仍在运行的 CLI 调用、整体或角色级高失败率进入 `needs_attention`。
  - 返回 `nextAction` 和 `nextCommands`，让 Codex 直接执行下一步，而不是只看原始 JSON。
- 新增 `tools/dionysus-system-audit.test.ts`
  - 覆盖 readiness 阻断、pending outbox 提醒、高失败率角色提醒、完全 ready 四类行为。
- `tools/dionysus.ts`
  - 接入 `pnpm dionysus system audit --target-root "<target-root>" [--goal-id "<goal-id>"] [--allow-dirty-path "..."]`。
  - CLI 会先执行 outbox reconcile，再拉取 usage / pending outbox / 可选 goal status。
- 文档同步：
  - `AGENTS.md` 增加 `system audit` 作为 Codex 日常首选产品化入口。
  - `docs/specs/api.md` 增加审计契约。
  - `features_test/dionysus-mvp.feature.md` 增加 BDD 场景 6.5.3。
  - `/Users/yangyu/.codex/skills/dionysus/SKILL.md` 同步更新，要求后续上下文恢复后优先使用审计入口。

### 验证证据

- `pnpm exec vitest run tools/dionysus-system-audit.test.ts tools/dionysus-readiness.test.ts tools/dionysus-command.test.ts` 通过，22/22。
- `pnpm typecheck` 通过。
- `pnpm test` 通过，51 个测试文件、251 个测试。
- 真实运行 `pnpm -s dionysus system audit --target-root /Volumes/MacMiniSSD/code/Coupon`：
  - readiness 为 `ready`，PostgreSQL / RabbitMQ / Worker / CLI 均可用。
  - 返回 `needs_attention`，原因是 `test_writer CLI 失败率偏高：12/21`。
  - pending Codex outbox 为 0。
  - 下一步命令为查看 Coupon target 的 Agent usage。

### 下一步

- 提交并推送本轮 Dionysus 改动。
- 提交后运行 `pnpm -s dionysus system runtime heal`，让正在运行的 API / Worker 加载新 commit。
- 后续推进 Coupon 前先运行 `system audit`；如果仍显示 TestWriter 失败率偏高，先查看最近失败 run，并决定是调整 TestWriter prompt / 模型，还是让 Codex 亲自接手测试方案。

## 2026-05-20 Dionysus Audit 历史失败恢复判断记录

### 背景

- 上一轮 `system audit` 能发现 TestWriter 失败率偏高，但它只看累计失败率，导致早期试跑失败会永久让 Coupon 目标返回 `needs_attention`。
- 本轮按 audit 建议查看了 TestWriter run 列表：
  - 失败主要集中在 2026-05-16 至 2026-05-17 的早期真实 DB smoke / COU-221 E2E 试跑。
  - 后续 TestWriter 已有成功 run，例如 `dbcf33ae-5437-4420-bb28-9737366b678d`、`dbe03028-a034-467f-99a6-9afedfcd2215`、`0e7da48a-9e3c-45a3-9d97-94fe78076d06`。
  - 结论：这里不是当前 runtime/CLI 不可用，而是历史失败债被 audit 当成当前风险。

### 完成内容

- `packages/core/src/agent-cli-usage.ts`
  - usage 聚合新增 `lastSucceededAt` 和 `lastFailedAt`，分别写到 role、agent instance、CLI model、CLI 汇总。
  - 这样 Codex 不只看到累计失败率，还能判断最后一次运行是否已经恢复。
- `packages/core/src/types.ts` 与 `apps/web/src/api.ts`
  - 同步新增 usage 时间字段类型。
- `tools/dionysus-system-audit.ts`
  - 新增 `notes` 输出。
  - 当某角色历史失败率偏高，但 `lastSucceededAt > lastFailedAt` 且最近运行已成功时，不再返回 `needs_attention`，而是把历史失败写入 `notes`。
  - pending outbox、当前运行中调用、真实模型证据缺口、未被后续成功覆盖的高失败率仍然保持 `needs_attention`。
- 文档同步：
  - `AGENTS.md`、`docs/specs/api.md`、`features_test/dionysus-mvp.feature.md`、`/Users/yangyu/.codex/skills/dionysus/SKILL.md` 均补充“历史失败恢复后不能永久阻断派工”的规则。

### 验证证据

- 先写红灯测试后实现：
  - `packages/core/src/agent-cli-usage.test.ts` 覆盖 last success / last failure 时间聚合。
  - `tools/dionysus-system-audit.test.ts` 覆盖历史高失败但最后成功时返回 `ready` 且写入 `notes`。
- `pnpm exec vitest run packages/core/src/agent-cli-usage.test.ts tools/dionysus-system-audit.test.ts tools/dionysus-readiness.test.ts tools/dionysus-command.test.ts` 通过，29/29。
- `pnpm typecheck` 通过。
- `pnpm test` 通过，51 个测试文件、253 个测试。

### 下一步

- 已提交并推送：`e636fe8 fix(audit): distinguish recovered CLI failures`。
- 已运行 `pnpm -s dionysus system runtime heal`，API / Worker 已重启到 commit `e636fe8406ea38312df16bc6431fa59d53976196`。
- 重启后真实运行 `pnpm -s dionysus system audit --target-root /Volumes/MacMiniSSD/code/Coupon`：
  - `status=ready`
  - `warnings=[]`
  - `notes=["test_writer 曾有较高历史失败率：12/21，但最近一次运行已成功"]`
  - `pendingCodexOutboxCount=0`
  - 下一步命令为 `fastlane coupon-module-plan`
- 当前 Dionysus 已不会因为已恢复的 TestWriter 历史失败永久阻断 Coupon 新模块派工。

## 2026-05-20 Dionysus Fast Lane report-only 模式记录

### 背景

- `system audit` 已返回 Coupon `ready`，下一步应使用 Dionysus 做 D1 里程碑验收缺口审计。
- 试跑普通 `fastlane plan` 发现控制面缺陷：审计任务本应只读输出报告，但默认 Worker prompt 仍要求“Produce the smallest reviewable patch”，会误导低成本 Agent 为审计任务生成无意义文件改动。
- 该问题会直接影响 Dionysus 作为 Agent Team 执行系统的可控性：不是所有 Worker 任务都应该产出 patch，模块验收、缺口扫描、方案评审需要 report-only lane。

### 完成内容

- `tools/dionysus-fastlane.ts`
  - `buildFastLanePlan()` 新增 `reportOnly` 输入。
  - 计划输出新增 `mode: "patch" | "report_only"`。
  - report-only Worker prompt 明确：
    - `Report-only mode`
    - 禁止修改文件、生成 patch、提交、push、reset 或直接编辑目标 main worktree。
    - `No patch is required`，交付物是带证据的报告。
    - 必须区分已证明完成、缺失、证据薄弱、矛盾、风险和下一 owner。
  - report-only Reviewer prompt 明确评审 Worker report，不要求 integration patch。
  - nextCommands 改为 `Worker report-only 产出后...`，避免继续使用 patch/integration 语义。
- `tools/dionysus.ts`
  - `fastlane plan` 和 `fastlane start` 支持 `--report-only`。
- 文档同步：
  - `AGENTS.md`
  - `docs/specs/api.md`
  - `features_test/dionysus-mvp.feature.md`
...
- 先写红灯测试：
  - `tools/dionysus-fastlane.test.ts` 新增 report-only fast lane 用例，确认 Worker 不再要求 patch、Reviewer 评审 report。
- `pnpm exec vitest run tools/dionysus-fastlane.test.ts tools/dionysus-system-audit.test.ts` 通过，26/26。
- `pnpm typecheck` 通过。
- `pnpm test` 通过，51 个测试文件、254 个测试。
- CLI 验证：
  - `pnpm -s dionysus fastlane plan --report-only ...`
  - 输出 `mode=report_only`
  - worker 描述包含 `Report-only mode` 和 `No patch is required`
  - reviewer 描述包含 `Worker report`
  - nextCommands 提示 `Worker report-only 产出后...`

### 下一步

- 提交并推送本轮 Dionysus 改动。
- 执行 `pnpm -s dionysus system runtime heal`。
- 使用 `fastlane start --report-only` 创建 D1 身份租户模块里程碑验收缺口审计目标，让 Dionysus 真实调度 WorkerCLI 产出只读审计报告。

## 2026-05-20 Report-only Reviewer 证据注入修复

### 背景

- 在 Coupon D1 验收缺口审计 goal `fe8d7b67-13d9-4b63-b7d0-bc291331ad49` 中，report-only ReviewerCLI 被取消。
- 根因不是 Worker 没有产出，而是 Reviewer 任务启动时没有拿到上游 Worker run logs / reports 作为一等证据，只能在 workspace 中重新探索，导致“审核 Worker 产物”变成“重复做一次代码调查”。
- 这违背 Dionysus 的核心控制力目标：便宜 CLI Worker 产出、便宜 CLI Reviewer 先筛、Codex 最终裁决。Reviewer 必须基于 Worker 产物评审，而不是无证据地自由发挥。

### 已完成

- `apps/api/src/server.ts`
  - 在 `POST /api/tasks`、`POST /api/tasks/:id/enqueue` 和 `dispatchNextTaskAfterReview` 自动启动 FastLane Reviewer 前，调用 `attachReportOnlyReviewerEvidenceIfNeeded`。
  - 对 report-only Reviewer 自动收集同一 goal 下状态为 `needs_review` / `done` 的 `FastLane Worker` 最新 run logs。
  - 将证据写入 Reviewer 任务事件 `reviewer.worker_reports_evidence`，包含 task id、title、status、run id、run status、finishedAt、logExcerpt。
  - run log excerpt 会去除 ANSI 和 `DIONYSUS_DONE_JSON` 标记，并做 head+tail 截断，保留最终报告尾部。
- `packages/core/src/role-prompt.ts`
  - Runtime 构建 prompt 时读取 `reviewer.worker_reports_evidence` 事件，注入 `Worker Report Evidence` 区块。
  - prompt 明确要求 Reviewer 优先审核 Worker 报告，不能用重新探索代码替代审核；如果没有 Worker 报告证据，必须 BLOCKED。
- `packages/core/src/role-prompt.test.ts`
  - 新增有 Worker 报告时 prompt 注入证据的测试。
  - 新增无 Worker 报告时提示 Reviewer BLOCKED 的测试。
- 已更新 `docs/specs/api.md`、`docs/specs/state-machine.md`、`features_test/dionysus-mvp.feature.md`，把该行为固化为 Dionysus 控制面契约。

### 验证

- `pnpm exec vitest run packages/core/src/role-prompt.test.ts tools/dionysus-fastlane.test.ts`：2 files / 30 tests passed。
- `pnpm typecheck`：通过。
- `pnpm test`：51 files / 256 tests passed。

### 下一步

- 提交并推送本轮 Dionysus 修复。
- 提交后运行 `pnpm -s dionysus system runtime heal`，确保 API / Worker runtime 使用最新 commit。
- 后续再启动 report-only fast lane 时，应检查 Reviewer prompt 或 task events，确认存在 `reviewer.worker_reports_evidence` 后再让 ReviewerCLI 审核。

## 2026-05-20 FastLane Reviewer reject 不再自动重排

### 背景

- 在 Coupon D1 `/identity` 认证与租户作用域错误态闭环目标中，ReviewerCLI 低分 BLOCKED 后，Codex 执行 `task review --verdict reject`，Dionysus 却把 Reviewer task 重新置为 `queued` 并重新分配 Agent。
- 这会导致同一个低质量 Reviewer 反复消耗便宜 CLI token，违背 Dionysus 的核心目标：便宜 Agent 并行产出，Codex 保证质量与成本控制。
- 正确产品语义：普通 Worker 被 reject 应返工；FastLane Reviewer 被 reject 表示 Codex 拒绝该质量门禁报告，应阻断并交给 Codex 裁决，不应自动重跑 ReviewerCLI。

### 已完成

- `packages/core/src/task-review.ts`
  - 新增 `isFastLaneReviewerTaskTitle()`。
  - 新增 `taskReviewStatusForContext()`：`FastLane Reviewer` + `reject` 映射为 `blocked`，普通 reject 仍为 `queued`。
  - 新增 `shouldRequeueRejectedTask()`：只有非 Reviewer reject 才重新投递。
- `apps/api/src/server.ts`
  - `/api/tasks/:id/review` 改为使用上下文敏感状态映射。
  - `FastLane Reviewer` 被 reject 时：
    - task 标记为 `blocked`。
    - 写入 `task.review_fastlane_reviewer_rejected`。
    - 写入 Codex Outbox `blocker`。
    - 不再 publish RabbitMQ 消息重排 ReviewerCLI。
- `packages/core/src/task-review.test.ts`
  - 新增 FastLane Reviewer reject 不重排测试。
  - 新增普通 Worker reject 仍 queued 返工测试。
- 文档 / BDD / Skill 同步：
  - `docs/specs/state-machine.md`
  - `docs/specs/api.md`
  - `features_test/dionysus-mvp.feature.md`
...
- `pnpm exec vitest run packages/core/src/task-review.test.ts tools/dionysus-fastlane.test.ts`：2 files / 30 tests passed。
- `pnpm typecheck`：通过。
- `pnpm test`：52 files / 266 tests passed。
- `git diff --check`：通过。

### 下一步

- 提交并推送本轮 Dionysus 控制面修复。
- 提交后运行 `pnpm -s dionysus system runtime heal`，确保 API / Worker 使用新 commit。
- 然后继续用 Dionysus fast lane 推进 Coupon D1 的下一个最终用户闭环。

## 2026-05-20 Codex 接手完成任务状态门禁修复

### 背景

- 继续推进 Dionysus 可用于真实 Coupon 开发的控制链闭环时，复核到 `task codex-complete` 的仓储实现仍允许把 `cancelled` task 更新为 `done`。
- 这会复活已经取消的工作，并可能触发后续任务分发，违背 Dionysus 状态机中“已取消任务不可复活”的核心约束。
- 用户最新要求开发必须从系统功能、系统设计和用户体验全局视角判断；本修复属于控制面产品语义修复：Codex 接手完成是人工修复入口，不是撤销历史取消决定的入口。

### 已完成

- `packages/core/src/state-machine.ts`
  - 新增 `codexCompletableTaskStatuses`。
  - 新增 `canCodexCompleteTaskStatus()`。
  - 允许 Codex 接手完成的状态为 `created`、`queued`、`assigned`、`running`、`needs_review`、`blocked`、`failed`。
  - 明确排除 `cancelled` 和 `done`。
- `packages/db/src/repository.ts`
  - `completeTaskByCodex()` 改为引用 core 白名单。
  - 不再允许 `cancelled` task 被改成 `done`。
  - 补上 `assigned` 作为可接手状态，避免任务卡在 assigned 时无法由 Codex 收口。
- 文档 / BDD / Skill 同步：
  - `docs/specs/state-machine.md`
  - `docs/specs/api.md`
  - `features_test/dionysus-mvp.feature.md`
...
- `pnpm exec vitest run packages/core/src/state-machine.test.ts`：1 file / 8 tests passed。
- `pnpm typecheck`：通过。
- `pnpm test`：51 files / 257 tests passed。
- `git diff --check`：通过。

### 下一步

- 已提交并推送：`17d70d3 fix(tasks): prevent codex completion of cancelled tasks`。
- 已运行 `pnpm -s dionysus system runtime heal`，旧 runtime commit `60fb7d7177c81f8d4a098815eb6d80d29c10efaf` 已重启到 `17d70d37be818df2206eb166174ae04d82649db2`。
- `pnpm -s dionysus system doctor --brief` 返回 API / PostgreSQL / RabbitMQ / Worker 正常；Worker effective run config 仍为 role_config `opencode` + `minimax-cn-coding-plan/MiniMax-M2.7`。
- `pnpm -s dionysus system audit --target-root /Volumes/MacMiniSSD/code/Coupon` 返回 `status=ready`、`blockers=[]`、`warnings=[]`、pending Codex Outbox 为 0。
- 下一步可以继续用 Dionysus fast lane 推进 Coupon 完整模块，优先选择最终用户可体验的 D1 闭环缺口。

## 2026-05-20 Coupon D1 report-only 审计目标收口记录

### 背景

- 用户要求 Codex 必须站在系统功能、系统设计和用户体验等产品经理全局视角开发，不能机械执行命令、机械复刻 HTML 模板或机械禁止跳转。
- 本轮使用 Dionysus report-only goal `61138ca9-dd19-4dd3-830b-05b301f0e4f2` 审计 Coupon D1 身份租户模块闭环。

### Dionysus 执行结果

- Worker1 / Worker2 总共提供 D1 功能地图与 E2E 证据线索。
- 当前 goal 累计 OpenCode / MiniMax-M2.7 调用 5 次，modelCalls 5 次，全部 succeeded。
- Reviewer1 因基于未完成 Worker1 报告且机械判断被取消。
- Reviewer2 任务 `c44ca919-5fa8-45cd-8197-5e105cf03d4c` 启动后没有输出要求的 `Verdict / Score / Product closure map / Coverage gaps / Required fixes`，日志最后停在“需要我继续做什么”。Codex 已执行：
  - `task review --verdict reject`
  - 随后 `task cancel`
- 审计 goal `61138ca9-dd19-4dd3-830b-05b301f0e4f2` 已由 Codex 明确 `goal cancel`，原因是 Worker 线索已被采用，但 Reviewer 门禁不合格，后续 D1 实现应另建新的最终用户闭环 fast lane。
- 结论：本轮 Dionysus Worker 输出可作为线索；Reviewer 输出未达到门禁质量，不能继续让该审计目标空转。Codex 已直接接管 D1 产品裁决。

### 对 Dionysus 的产品反馈

- report-only Reviewer 即使拿到了证据注入，也仍可能转成自由探索并不输出最终裁决。
- 后续 Dionysus 应加强 Reviewer 完成门禁：如果 Reviewer 最终输出缺少指定结构字段或 `DIONYSUS_DONE_JSON` 前没有 Verdict/Score，应自动标记 failed / blocked，而不是进入 `needs_review`。
- 这属于 Dionysus 控制力问题：低成本 Reviewer 只能先筛，不能把未完成报告交给 Codex 当作有效门禁。

### Coupon 侧实际产出

- Codex 在 Coupon 直接修复 `apps/admin-web/src/pages/identity.vue` 的支持授权摘要 Vue key 稳定性问题。
- 该修复不属于 Dionysus 自动 patch，而是 Codex 在 Reviewer 失败后按产品巡检直接接手的小范围修复。
- Coupon 侧验证：
  - identity overview E2E：6 passed。
  - members / support-grants / audit E2E：27 passed。
  - admin-web build：passed。

### 下一步

- Dionysus 继续可用于 WorkerCLI 批量探索和实现，但 Reviewer 门禁必须由 Codex 严格审核。
- 下一轮 Coupon D1 目标应围绕最终用户可体验闭环：
  1. 无登录态 / 缺租户上下文时 identity 页面不能静默显示 0 数据。
  2. 支持授权和代操作会话需要形成从授权到代操作到审计详情的完整用户链路。
  3. 审计详情页需要产品化展示，避免只保留技术字段。

## 2026-05-20 Report-only Reviewer 输出结构门禁

### 背景

- 上一轮 Coupon D1 report-only 审计中，Reviewer2 的 CLI 进程正常退出，但没有输出 `Verdict / Score / Product closure map / Coverage gaps / Required fixes` 等最终裁决字段，最后停在“需要我继续做什么”。
- 这说明只靠 prompt 和人工 review score 不够。Dionysus Runtime 必须在任务状态转换前识别“未完成 Reviewer 报告”，否则 Dashboard 和 Codex 会被 `needs_review` 状态误导。

### 已完成

- 新增 `packages/core/src/reviewer-output-gate.ts`：
  - 只门禁 `FastLane Reviewer` 且 description 包含 `Report-only mode` 的任务。
  - 要求输出包含 `Verdict:`、`Score:`、`Evidence reviewed:`、`Coverage gaps:`、`Required fixes:`、`Codex handoff:`。
  - `Verdict` 只接受 `PASS` / `BLOCKED`。
  - `Score` 只接受 0-100 的数字。
- 新增 `packages/core/src/reviewer-output-gate.test.ts`：
  - 覆盖非 Reviewer / patch-mode Reviewer 不受影响。
  - 覆盖“需要我继续做什么”这类未完成输出会被 block。
  - 覆盖 invalid verdict / score 会被 block。
  - 覆盖完整 BLOCKED/PASS 输出会通过。
- `apps/worker/src/worker.ts`
  - 在 Worker Runtime 完成 CLI run 后、生成 patch 前执行 report-only Reviewer 输出门禁。
  - 门禁失败时把 effective exit code 改为 1，写入 `reviewer.output_gate_failed` task event，不生成 patch，不让 task 进入 `needs_review`。
- 文档同步：
  - `docs/specs/state-machine.md`
  - `docs/specs/api.md`
  - `features_test/dionysus-mvp.feature.md`
...
- 先运行红灯测试：`pnpm exec vitest run packages/core/src/reviewer-output-gate.test.ts`，因实现缺失失败。
- 实现后验证：
  - `pnpm exec vitest run packages/core/src/reviewer-output-gate.test.ts`：5 passed。
  - `pnpm exec vitest run packages/core/src/reviewer-output-gate.test.ts packages/core/src/task-review.test.ts packages/core/src/state-machine.test.ts`：20 passed。
  - `pnpm typecheck`：通过。
  - `pnpm test`：52 files / 262 tests passed。
  - `git diff --check`：通过。

### 下一步

- 提交并推送本轮 Dionysus 控制面修复。
- 提交后运行 `pnpm -s dionysus system runtime heal`，确保 API / Worker 使用带结构门禁的新 commit。
- 之后再启动 Coupon D1 实现型 fast lane，优先处理 `/identity` 无登录态 / 缺租户上下文静默 0 数据的问题。

## 2026-05-20 FastLane Reviewer 产品/UX 输出门禁

### 背景

- 用户明确要求后续开发必须站在系统功能、系统设计、用户体验等产品经理全局视角开发，不能单纯教条执行命令、机械复刻 HTML 模板或机械禁止所有跳转。
- 复核 Dionysus 发现：Coupon 页面 fast lane 已经会把模板一致性、产品语义、页内上下文切换 vs 明确 CTA 路由/弹窗写入任务描述，但 Runtime 只对 `--report-only` Reviewer 做结构化输出门禁。
- 这意味着普通实现型 Reviewer 仍可能只输出“测试通过 / 页面像模板 / 需要我继续做什么”并进入 `needs_review`，让 Codex 和 Dashboard 被低质量 Reviewer 报告误导。

### 已完成

- `packages/core/src/reviewer-output-gate.ts`
  - 新增 `evaluateFastLaneReviewerOutputGate()`，覆盖所有 `FastLane Reviewer`。
  - 普通实现型 Reviewer 必须输出 `Verdict:`、`Score:`、`Evidence:`、`Product/UX assessment:`、`Required fixes:`、`Codex handoff:`。
  - `Product/UX assessment` 成为硬门禁，必须从最终用户任务流、信息架构、页内上下文切换 vs 明确 CTA 路由/弹窗、视觉/模板适配等角度给出判断。
  - 保留 `evaluateReportOnlyReviewerOutputGate()` 旧语义，只门禁 report-only Reviewer，避免破坏外部兼容。
- `apps/worker/src/worker.ts`
  - Runtime 改用 `evaluateFastLaneReviewerOutputGate()`。
  - 门禁失败时写入 `reviewer.output_gate_failed`，task 不进入 `needs_review`。
- `tools/dionysus-fastlane.ts`
  - 普通 Reviewer prompt 的 required response format 增加 `Product/UX assessment:`。
- 文档 / BDD / Skill 同步：
  - `docs/specs/state-machine.md`
  - `docs/specs/api.md`
  - `features_test/dionysus-mvp.feature.md`
...
- 先写红灯测试，确认新函数缺失时 `packages/core/src/reviewer-output-gate.test.ts` 失败。
- 实现后验证：
  - `pnpm exec vitest run packages/core/src/reviewer-output-gate.test.ts tools/dionysus-fastlane.test.ts`：2 files / 28 tests passed。
  - `pnpm typecheck`：通过。

### 下一步

- 运行 `pnpm test`、`git diff --check`。
- 提交并推送本轮 Dionysus 控制面修复。
- 提交后运行 `pnpm -s dionysus system runtime heal` 和 `pnpm -s dionysus system doctor --brief`，确保 Worker Runtime 使用新门禁。

## 2026-05-20 FastLane Reviewer reject API 回归测试

### 背景

- 用户继续强调开发必须站在系统功能、系统设计和用户体验等产品经理全局视角，不要机械执行命令或教条复刻页面。
- 对 Dionysus 来说，关键控制面语义是：低成本 ReviewerCLI 可以辅助筛查，但 Reviewer 被 Codex 明确 reject 后，系统必须停止自动重派，转入 Codex 产品裁决，而不是继续消耗 CLI 调用空转。
- 上一轮已经修复 core 层策略，本轮补 API route 级回归，防止 `/api/tasks/:id/review` 未来绕过策略。

### 已完成

- `apps/api/src/server.ts`
  - `buildServer()` 新增可测试注入选项：`repo`、`publishJson`、`logger`。
  - 路由内部发布 RabbitMQ 消息时优先使用注入的 publisher，便于 API 级测试断言“是否重新入队”。
  - 注入 repo 时不再创建真实 PostgreSQL pool，测试关闭 server 时也不会误关不存在的 pool。
- `apps/api/src/task-review-route.test.ts`
  - 新增 API 级测试：FastLane Reviewer 被 reject 后必须返回 `blocked`、创建 Codex outbox blocker、记录事件，并且不发布任何重新入队消息。
  - 新增对照测试：普通 FastLane Worker 被 reject 后仍应进入 `queued`，并发布到 `dionysus.worker` 进行下一轮迭代。
- `packages/db/package.json` / `pnpm-lock.yaml`
  - 补上 `@dionysus/db` 对 `@dionysus/core` 的 workspace dependency。此前 db 代码已 import core，但包声明缺失，会导致 API 测试和运行时包解析不稳定。

### 验证

- `pnpm exec vitest run apps/api/src/task-review-route.test.ts packages/core/src/task-review.test.ts`：2 files / 11 tests passed。
- `pnpm typecheck`：通过。
- `pnpm test`：53 files / 268 tests passed。
- `git diff --check`：通过。

### 下一步

- 提交并推送本轮 Dionysus 控制面修复。
- 提交后运行 `pnpm -s dionysus system runtime heal`、`pnpm -s dionysus system doctor --brief` 和 `pnpm -s dionysus system audit --target-root /Volumes/MacMiniSSD/code/Coupon`。
- 然后继续以“最终用户完整功能模块”为单位推进 Coupon，UI/UX 判断按业务路径，而不是机械套 HTML 模板。

## 2026-05-20 system audit 最近失败恢复门禁

### 背景

- 继续 Dionysus 长期目标时重新运行真实状态检查，`pnpm -s dionysus system audit --target-root /Volumes/MacMiniSSD/code/Coupon` 在旧逻辑下返回 `ready`。
- 但同一份 usage evidence 显示 Master 和 Worker 最近一次 CLI run 都是失败，且失败时间晚于最后成功时间。
- 这类状态不应直接推荐继续派工；Codex 应先查看最近失败原因，判断是已接手的历史失败、prompt / 模型问题，还是需要暂停扩并发。

### 已完成

- `tools/dionysus-system-audit.ts`
  - 新增 `hasUnrecoveredLatestFailure()`。
  - 当整体或某个角色 `lastRunAt` 指向最近失败，且 `lastSucceededAt` 早于 `lastFailedAt` 时，`system audit` 进入 `needs_attention`。
  - `nextAction` 明确要求先查看最近失败的 Agent 运行日志，确认是否可忽略、是否已由 Codex 接手，或是否要调整 prompt / CLI 模型。
- `tools/dionysus-system-audit.test.ts`
  - 新增回归测试：即使历史失败率低于 30%，只要角色最近一次 run 失败且没有后续成功恢复，就不能返回 `ready`。
- 文档 / Skill 同步：
  - `docs/specs/api.md`
  - `AGENTS.md`
  - `/Users/yangyu/.codex/skills/dionysus/SKILL.md`
...
- 红灯：新增测试先失败，旧逻辑返回 `ready`。
- 绿灯：`pnpm exec vitest run tools/dionysus-system-audit.test.ts` 通过，6 tests passed。
- 全量验证：
  - `pnpm typecheck`：通过。
  - `pnpm test`：53 files / 269 tests passed。
- 真实状态验证：
  - `pnpm -s dionysus system audit --target-root /Volumes/MacMiniSSD/code/Coupon` 现在返回 `needs_attention`。
  - warnings 包含：
    - `master 最近一次 CLI 运行失败，尚无后续成功恢复证据`
    - `worker 最近一次 CLI 运行失败，尚无后续成功恢复证据`

### 下一步

- 提交并推送本轮 Dionysus 控制面修复。
- 提交后运行 `pnpm -s dionysus system runtime heal` 和 `pnpm -s dionysus system doctor --brief`，确保 API / Worker 使用新 audit 规则。
- 后续继续推进 Coupon 前，应先处理 audit 中最近失败的 Master / Worker 证据，再启动新的 fast lane。

### 后续修正

- 查询最近 run 后确认，当前 Master / Worker 的“最近失败”来自已经被 Codex 接手、发布或取消的历史目标，主要原因是 Codex 在只读审计 Worker 运行期间修改了 Coupon 目标工作区，Dionysus 正确阻断了该 run。
- 为避免已关闭目标的历史失败长期污染当前派工判断，继续修复 usage / audit 口径：
  - `buildAgentCliUsageSummary()` 新增 `activeGoalRunTracking` 与 `latestActiveRunAt/latestActiveSucceededAt/latestActiveFailedAt`。
  - PostgreSQL usage 查询保留全历史 `cliCalls/modelCalls/failedCalls`，同时把 `g.status` 传入 usage builder。
  - `system audit` 在判断“最近一次失败未恢复”时优先使用未关闭 goal 的 active timestamps；`done/failed/cancelled` goal 的失败只保留在成本统计中，不作为当前 blocker。
- 新增测试：
  - closed goal 的失败仍计入总调用/失败数，但不设置 active failure。
  - active goal 已恢复时，audit 不再因历史 closed-goal failure 返回 `needs_attention`。
- 追加验证：
  - `pnpm exec vitest run packages/core/src/agent-cli-usage.test.ts tools/dionysus-system-audit.test.ts`：2 files / 14 tests passed。
  - `pnpm typecheck`：通过。
  - `pnpm test`：53 files / 271 tests passed。

## 2026-05-20 API Runtime commit 证据与 heal 门禁

### 背景

- 继续推进 Dionysus 主目标时复查运行态：`system doctor --brief` 显示 API / PostgreSQL / RabbitMQ / Worker 正常，Worker effective config 为 OpenCode + MiniMax-M2.7。
- 但 runtime 自愈只比较 Worker 心跳中的 `worker.runtime.codeCommitSha` 与当前仓库 HEAD；如果 API 进程仍运行旧提交，`release_ready`、Codex Outbox、review gate、system audit 等 API 侧门禁可能仍是旧逻辑。
- 这会造成 Codex 误判“新门禁已生效”，实际请求仍打到旧 API。

### 已完成

- `apps/api/src/server.ts`
  - `/health` 新增 `runtime` 元数据：`pid`、`runtimeInstanceId`、`runtimeStartedAt`、`codeCommitSha`。
  - `buildServer()` 增加测试注入项 `checkRabbitMqHealth` 和 `apiCodeCommitSha`，便于 API route 级测试不依赖真实 RabbitMQ / git。
- `tools/dionysus-runtime.ts`
  - `buildRuntimeHealPlan()` 新增 API commit 比较。
  - 当 `/health.runtime.codeCommitSha` 落后于当前 Dionysus HEAD 时，返回 `restart`，reason 为 `api runtime commit stale: old != new`。
  - 保留 Worker commit stale 检测。
  - 后续真实 `runtime heal` 暴露出 partial runtime 问题：Worker 缺失但 API 仍运行时，旧逻辑只补 Worker，可能保留旧 API。本轮继续修正为“部分进程缺失时整体 restart”，确保 API/Worker 来自同一提交。
- `apps/api/src/health-route.test.ts`
  - 新增 `/health` API route 测试，证明 API runtime commit metadata 会返回给 doctor / runtime heal。
- `tools/dionysus-runtime.test.ts`
  - 新增 API 旧提交触发 runtime restart 的红绿灯测试。
  - 新增 partial runtime 整体 restart 和全进程停止时 start 的回归测试。
- 文档 / BDD / Skill 同步：
  - `docs/specs/api.md`
  - `features_test/dionysus-mvp.feature.md`
  - `AGENTS.md`
...
- 红灯：新增 `tools/dionysus-runtime.test.ts` 用例先失败，旧逻辑返回 `runtime healthy`。
- 绿灯：
  - `pnpm exec vitest run apps/api/src/health-route.test.ts tools/dionysus-runtime.test.ts tools/dionysus-doctor.test.ts`：3 files / 11 tests passed。
  - partial runtime 修复后 `pnpm exec vitest run tools/dionysus-runtime.test.ts`：1 file / 10 tests passed。
  - `pnpm typecheck`：通过。
  - `pnpm test`：54 files / 274 tests passed。
  - `git diff --check`：通过。

### 下一步

- 提交并推送本轮 Dionysus 控制面修复。
- 提交后运行 `pnpm -s dionysus system runtime heal`，确保 API 与 Worker 都重启到新 commit。
- 再运行 `pnpm -s dionysus system doctor --brief` 和 `pnpm -s dionysus system audit --target-root /Volumes/MacMiniSSD/code/Coupon` 复验。

## 2026-05-20 Milestone E2E 状态机硬门禁

### 背景

- 继续推进 Dionysus 主目标时，真实 `doctor` 和 `system audit --target-root /Volumes/MacMiniSSD/code/Coupon` 均显示 ready，Coupon 工作区 clean，说明系统可继续派工。
- 复查里程碑链路时发现：`request-e2e` 和 `create-campaign` 主要靠 SQL `where status = ...` 静默过滤，没有在 Repository / API 层显式校验 milestone 状态转换。
- 风险是已通过、已取消、已运行中或异常状态的 milestone 可能返回 `202/201` 假成功，甚至误写 Codex Outbox，削弱“里程碑必须由 Codex 浏览器 E2E 验收”的可信度。

### 已完成

- `packages/core/src/milestone-orchestration.ts`
  - 新增 `milestoneStatusForE2ERequest()`，只允许 `candidate -> e2e_required`。
  - 新增 `milestoneStatusForE2ECampaignCreation()`，只允许 `e2e_required -> e2e_running`。
- `packages/db/src/repository.ts`
  - `requestMilestoneE2E()` 先读取 milestone 当前状态并调用核心状态机；非法状态直接抛出 `Invalid milestone transition`，不再静默更新 0 行。
  - `createE2ECampaign()` 只允许从 `e2e_required` 创建 campaign；非法状态回滚 campaign / cases 写入。
- `apps/api/src/server.ts`
  - `POST /api/milestones/:id/request-e2e` 和 `POST /api/milestones/:id/e2e-campaigns` 对非法状态返回 `409 INVALID_MILESTONE_TRANSITION`。
  - milestone 不存在时返回 `404 MILESTONE_NOT_FOUND`。
- 新增 `apps/api/src/milestone-route.test.ts`：
  - 覆盖非法状态下 `request-e2e` 不能假成功。
  - 覆盖非法状态下 `e2e-campaigns` 不能创建 campaign。
- 文档 / BDD / Skill 同步：
  - `docs/specs/e2e-and-notification.md`
  - `docs/specs/api.md`
  - `features_test/dionysus-mvp.feature.md`
...
- 红灯：`pnpm exec vitest run packages/core/src/milestone-orchestration.test.ts apps/api/src/milestone-route.test.ts` 先失败，核心函数缺失且 API 返回 500。
- 绿灯：
  - `pnpm exec vitest run packages/core/src/milestone-orchestration.test.ts apps/api/src/milestone-route.test.ts packages/core/src/e2e-results.test.ts`：3 files / 15 tests passed。
  - `pnpm typecheck`：通过。
  - `pnpm test`：55 files / 278 tests passed。
  - `git diff --check`：通过。

### 下一步

- 提交并推送本轮 Dionysus milestone 控制面修复。
- 提交后运行 `pnpm -s dionysus system runtime heal`，确保 API / Worker 使用新状态机门禁。
- 复验 `pnpm -s dionysus system doctor --brief` 与 `pnpm -s dionysus system audit --target-root /Volumes/MacMiniSSD/code/Coupon`。

### 提交后状态

- 已提交并推送：`c577fc9 fix(milestones): enforce e2e state transitions`。
- `pnpm -s dionysus system runtime heal` 已重启 runtime，原因是旧 API commit `0dd79c0...` 落后于当前 `c577fc90f8cadbcb4b66d22c69f3d3975acf5a0a`。
- `pnpm -s dionysus system doctor --brief`：API / PostgreSQL / RabbitMQ / Worker 均 ok，API 与 Worker `codeCommitSha` 均为 `c577fc90f8cadbcb4b66d22c69f3d3975acf5a0a`。
- `pnpm -s dionysus system audit --target-root /Volumes/MacMiniSSD/code/Coupon`：`ready`，无 blockers / warnings，pending Codex outbox 为 0，Coupon 工作区 clean。

## 2026-05-20 Release Record 与 Codex Outbox 关联门禁

### 背景

- 继续推进 Dionysus 目标时重新复验：`doctor --brief` 正常，Coupon audit 为 `ready`，pending Codex outbox 为 0。
- 复查 release 闭环发现：`POST /api/releases` 已经验证 release record 的证据质量，也要求 `release_ready` ack 前必须存在 release record，但没有验证 `release record.codexOutboxEventId` 是否真的属于同一个 goal 的 pending `release_ready`。
- 风险是错误 goal 的 release record、已 ack 事件或 blocker/e2e outbox id 可能被误关联，进而解除另一个 `release_ready` 的 ack 门禁，削弱 Codex 发布闭环证据链。

### 已完成

- `packages/core/src/release-record.ts`
  - 新增 `validateReleaseRecordCodexOutboxLink()`。
  - 要求 outbox 事件必须同时满足：`goalId` 与 release goal 一致、`eventType=release_ready`、`status=pending`。
- `apps/api/src/server.ts`
  - `POST /api/releases` 在传入 `codexOutboxEventId` 时先读取 outbox 事件。
  - 事件不存在返回 `404 CODEX_OUTBOX_EVENT_NOT_FOUND`。
  - 事件 goal/type/status 不匹配返回 `409 RELEASE_RECORD_OUTBOX_MISMATCH`，不写 release record。
- 新增 `apps/api/src/release-route.test.ts`：
  - 正确的 same-goal pending `release_ready` 可以创建 release record。
  - 其他 goal 的 `release_ready` 被拒绝。
  - 非 `release_ready` outbox 被拒绝。
- 文档 / BDD / Skill 同步：
  - `docs/specs/api.md`
  - `features_test/dionysus-mvp.feature.md`
  - `/Users/yangyu/.codex/skills/dionysus/SKILL.md`
...
- 红灯：`pnpm exec vitest run packages/core/src/release-record.test.ts apps/api/src/release-route.test.ts` 先失败，核心函数缺失且 API 返回 500。
- 绿灯：
  - `pnpm exec vitest run packages/core/src/release-record.test.ts apps/api/src/release-route.test.ts packages/core/src/codex-outbox.test.ts tools/dionysus-release-record.test.ts`：4 files / 25 tests passed。
  - `pnpm typecheck`：通过。
  - `pnpm test`：56 files / 282 tests passed。
  - `git diff --check`：通过。

### 下一步

- 提交并推送本轮 Dionysus release/outbox 关联门禁修复。
- 提交后运行 `pnpm -s dionysus system runtime heal`，再复验 `doctor --brief` 与 Coupon `system audit`。

### 提交后状态

- 已提交并推送：`5b2c585 fix(release): validate outbox linkage`。
- `pnpm -s dionysus system runtime heal` 已重启 API / Worker，原因是旧 API commit `72d70b7...` 落后于 `5b2c58517b91bdf403ee4413a3595a3fe258dca7`。
- `pnpm -s dionysus system doctor --brief`：API / PostgreSQL / RabbitMQ / Worker 均 ok，API 与 Worker `codeCommitSha` 均为 `5b2c58517b91bdf403ee4413a3595a3fe258dca7`。
- `pnpm -s dionysus system audit --target-root /Volumes/MacMiniSSD/code/Coupon`：`ready`，无 blockers / warnings，pending Codex outbox 为 0，Coupon 工作区 clean。

## 2026-05-20 Release Record summary 审计门禁

### 背景

- 继续推进 Dionysus 主目标时，当前工作区已有未提交红灯测试，指向 release record 审计漏洞。
- 旧逻辑只要求 `status=passed && pushed=true` 时提供 `summary`、changed files 和验证命令；`status=failed` 或 `status=blocked` 可以空 summary 写入。
- 这会让 Codex 在最终验证失败或目标阻塞时只留下状态，没有失败原因或阻塞原因，不利于后续恢复、用户汇报和审计。

### 已完成

- `packages/core/src/release-record.ts`
  - `validateReleaseRecordEvidence()` 现在要求所有非 passed/pushed 的 release record 也必须提供非空 `summary`。
  - 对 passed/pushed release record 保留原组合门禁：changedFiles、至少一条 passed verification、summary 必须同时存在。
- `packages/core/src/release-record.test.ts`
  - 新增 failed / blocked release record 的 summary 审计测试。
- `tools/dionysus-release-record.test.ts`
  - 新增 CLI builder 测试，证明 `pnpm dionysus release record --status blocked` 缺少 `--summary` 会被拒绝。
- 文档 / BDD / Skill 同步：
  - `docs/specs/api.md`
  - `features_test/dionysus-mvp.feature.md`
  - `/Users/yangyu/.codex/skills/dionysus/SKILL.md`
...
- 红灯：`pnpm exec vitest run packages/core/src/release-record.test.ts tools/dionysus-release-record.test.ts` 先失败，旧逻辑允许 blocked 空 summary。
- 绿灯：
  - `pnpm exec vitest run packages/core/src/release-record.test.ts tools/dionysus-release-record.test.ts`：2 files / 16 tests passed。
  - `pnpm exec vitest run packages/core/src/release-record.test.ts apps/api/src/release-route.test.ts packages/core/src/codex-outbox.test.ts tools/dionysus-release-record.test.ts`：4 files / 27 tests passed。
  - `pnpm typecheck`：通过。
...
- 提交并推送本轮 release summary 门禁修复。
- 提交后运行 `pnpm -s dionysus system runtime heal`，再复验 `doctor --brief` 和 Coupon `system audit`。
