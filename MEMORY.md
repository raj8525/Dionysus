...
- 提交并推送本轮改动。
- 提交后运行 `pnpm -s dionysus system runtime heal`。
- 重启后再次运行 `pnpm -s dionysus system audit --target-root /Volumes/MacMiniSSD/code/Coupon`，确认 TestWriter 历史失败进入 `notes`，Coupon 目标不再因为已恢复的历史失败而停留在 `needs_attention`。
