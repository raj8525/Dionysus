...
启动时只有数据基座 Worker 会立即入队。只读 API 和 Vue Worker 会先创建但保持 `created`，等数据基座完成、Codex approve 后，再由 `fastlane status` 给出入队命令。这样可以保留后续 API/Vue 并行速度，同时强制第一步必须先有完整数据库虚拟数据。
