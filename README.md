...
- Codex Outbox Dashboard：前端展示 pending `blocker` / `e2e_required` / `release_ready` / `user_notify`，提供建议命令与 ack；`release_ready` 在没有 release record 前禁用普通 ack。
...
启动时只有数据基座 Worker 会立即入队。只读 API 和 Vue Worker 会先创建但保持 `created`，等数据基座完成、Codex approve 后，API 会自动并发派发这两个后续 Worker；如果需要人工处理，`fastlane status` 也会给出两个入队命令。API 层会阻断提前 `task enqueue`；数据基座未完成时返回 `COUPON_DATA_FIRST_GATE_BLOCKED`。这样可以保留后续 API/Vue 并行速度，同时强制第一步必须先有完整数据库虚拟数据。

如果目标只是补充主 PostgreSQL 测试数据或 seed，而不是启动一个完整模块读闭环，使用 `--data-only`。该模式只生成“数据基座 Worker + 数据基座 ReviewerCLI”，不会创建 API Worker 或 Vue Worker，避免一次简单数据修复被扩张成页面和接口改造。
...
`--data-only` 的 Worker prompt 会强制要求：

- 验收命令以目标项目根目录为准，不得把 Dionysus 隔离 workspace 路径写进长期文档。
...
Coupon 管理后台页面任务有一条固定约束：`tenants.vue` 是成熟集团租户管理页，`hotels.vue` 是真实酒店门店/部门页；两者不再按 HTML 模板整页重写。其他页面迁移 Vue 时参考 `apps/admin-web/html/` 对应模板，但必须重写为响应式数据、接口调用、loading、error、empty state 和事件处理都完整的 Vue 页面，禁止通过 `v-html`、raw import 或长字符串注入 HTML。
...
如果目标项目仍有已识别归属、不能混入本次发布的既有改动，CLI 入口应显式传入允许路径：
